  ...
  "AccountService" can {
    ...
    "createAccount" should {
      "allow to add a new account to an existing customer" in new WithMocks {
        (mockCustomerRepository.getCustomer(_:CustomerId)).expects(customerIdFixture).returning(Some(customerFixture)).once()
        (mockCustomerRepository.setCustomer(_:Customer)).expects(*).once()
        (mockLedgerService.dispatchOperation(_: Deposit)(_:ClassTag[Deposit])).expects(*,*).returning(Future.successful(Right(depositFixture))).once()
        (() =>mockTimeBasedGenerator.generate).expects().returning(accountId1Fixture.value).twice()
        val Right(accountId) = Await.result(accountService.createAccount(customerIdFixture, Money.Dolar), 100.milliseconds)
